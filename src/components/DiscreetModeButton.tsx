...
// Eye icon SVG paths for visibility states
...
const DiscreetModeButton: React.FC<Props> = ({ 
  size = 20, 
  isVisible = true 
}) => {
  const theme = useTheme() as any; // Type assertion for custom theme
  const colors = theme.colors || theme;

  return (
    <TouchableOpacity
      activeOpacity={0.7}
      testID="discreet-mode-button"
      style={{
        padding: 4, // Touchable area padding
        alignItems: 'center',
    ...
    >
      ...
      >
        <Path
          d={isVisible ? EyeOpenPath : EyeClosedPath}
